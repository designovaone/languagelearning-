...
| **Band 1 now opens on function words** | The consequence of the fix above: Italian starts `e, di, il, la, che`, English `the, to, a, of`. As word cards these teach little, and `il / la / lo / le / gli` are five cards all meaning "the". Not fixed, because the assessment seeds them as known before the drill runs (M3 precedes M4). Watch it on the first real session | M3, then judge |
...
| **Translations are a ranked shortlist, not a chosen sense** | Stage 5 is **written and tested but not yet run** — it needs an `OPENROUTER_API_KEY` and ~€1–3. Until it runs, the card shows stage 4's top-ranked translation, which is usually right. The loader treats the stage 5 artifact as optional and says so when it is missing | stage 5, on a key |
| **Italian translations are often definitions, not words** | Found while building stage 5: 56.3% of Italian first-translations are a gloss (`atmosfera` → "atmosphere (all meanings), air") against 0.2% of the English ones. Stage 5's free deterministic pass fixes most of it; the loaded deck still carries the raw form until that stage runs | stage 5 |
